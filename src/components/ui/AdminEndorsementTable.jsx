    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      {students.length > 0 ? (
                ...
                <tr key={student.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      ...
                      {student.status === 'ready' ? 'Ready' : student.status === 'pending' ? 'Pending' : student.status === 'rejected' ? 'Rejected' : 'Completed'}
                    </span>
                  </td>
        ...
        <div className="py-20 flex flex-col items-center justify-center text-gray-400 text-center">
          ...
          <p className="text-base font-semibold text-gray-500 uppercase tracking-wide">No Endorsement Requests</p>
          <p className="text-sm mt-1">There are no students requesting endorsement at this time.</p>
        </div>
      )}
    </div>
  )
